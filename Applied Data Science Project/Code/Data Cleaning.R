...
F90S <- F90S%>%
  mutate(state=get_state_abb[State]
  )
C90S <- F90S %>%
  group_by(Candidate)
...
FS <- FS %>%
  mutate(last_name=Candidate)
FS <- FS %>%
  mutate(first_name=Candidate)
FS <- FS %>%
  mutate(first_name=sub('.*,\\s*','', FS$first_name))
FS <- FS %>%
  mutate(last_name=sub('\\s*,.*','', FS$last_name))
FS <- FS %>%
  mutate(first_name=sub("\\s+\\S+$", '', FS$first_name))
FS <- FS %>%
  mutate(first_name=sub("\\s+\\S+$", '', FS$first_name))
FS <- FS %>%
  mutate(first_name=sub("\\s+\\S+$", '', FS$first_name))
...
FS <- FS %>%
  mutate(party=sub("Democratic-Farm-Labor", 'democrat', FS$party))
FS <- FS %>%
...
RS <- RS %>%
  mutate(last_name=candidate)
RS <- RS %>%
  mutate(last_name=sub(".*? ", "", RS$last_name))
RS <- RS %>%
  mutate(last_name=sub(".*? ", "", RS$last_name))
RS <- RS %>%
  mutate(last_name=sub(".*? ", "", RS$last_name))
...
RSREF<-RSREF%>%
  mutate(previous_percentage=candidatevotes/totalvotes)
RSREF<-RSREF%>%
  filter(state !="AK")%>%
  filter(state !="WA")%>%
  filter(state !="CA")%>%
...
# Add Partisan Voting Data ------------------------------------------------

#Calculate Demonstrated PVI's from previous results and current results
Senate3 <- Senate2%>%
...
Senate4 <- Senate4%>%
  mutate(share_of_spent=spent/total_spent)
